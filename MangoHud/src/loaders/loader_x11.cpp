...
bool libx11_loader::Load(const std::string& library_name) {
  ...
  XGrabKeyboard =
      reinterpret_cast<decltype(this->XGrabKeyboard)>(
          dlsym(library_, "XGrabKeyboard"));
  if (!XGrabKeyboard) {
    CleanUp(true);
    return false;
  }
  ...
  XUngrabKeyboard =
      reinterpret_cast<decltype(this->XUngrabKeyboard)>(
          dlsym(library_, "XUngrabKeyboard"));
  if (!XUngrabKeyboard) {
    CleanUp(true);
    return false;
  }
  ...
  XDefaultRootWindow =
      reinterpret_cast<decltype(this->XDefaultRootWindow)>(
          dlsym(library_, "XDefaultRootWindow"));
  if (!XDefaultRootWindow) {
    CleanUp(true);
    return false;
  }
...
void libx11_loader::CleanUp(bool unload) {
  ...
  XGrabKeyboard = NULL;
  XUngrabKeyboard = NULL;
  XDefaultRootWindow = NULL;
