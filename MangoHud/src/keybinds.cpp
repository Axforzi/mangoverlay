...
Clock::time_point last_f2_press, toggle_fps_limit_press, toggle_preset_press, last_f12_press, reload_cfg_press, last_upload_press, menu_press;
...
void check_keybinds(struct overlay_params& params){
   ...
   auto elapsedMenu = now - menu_press;
   ...
   gamepad_poll();
   if (elapsedMenu >= keyPressDelay &&
       (keys_are_pressed(real_params->open_menu) || gamepad_chord_pressed())) {
      menu_press = now;
      menu_toggle_open();
   }

   if (menu_is_open()) {
      /* While the menu is open all other keybinds are suppressed so the
       * navigation keys (and Enter/Esc) cannot trigger unrelated actions. */
...
current_preset = real_params->preset[++i%size];
          mangohud_dbg("KEYBIND_PRESET: next=%d", (int)current_preset);
   ...
   if (elapsedF12 >= keyPressDelay &&
      ...
      mangohud_dbg("KEYBIND_F12: no_display -> %d menu=%d", real_params->no_display, menu_is_open());
   }
...
parse_overlay_config(&params, getenv("MANGOHUD_CONFIG"), false);
       mangohud_dbg("KEYBIND_RELOAD_CFG: after parse");
