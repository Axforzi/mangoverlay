...
#define parse_open_menu             parse_string_to_keysym_vec
...
static void set_param_defaults(struct overlay_params *params){
   /* Unified overlay starts with the HUD hidden; the menu (Shift+F9)
    * still works because check_keybinds runs before the no_display gate
    * in compute_swapchain_display. Toggle on with the menu or
    * MANGOHUD_CONFIG=no_display=0. */
   params->no_display = true;
   mangohud_dbg("PARAM_DEFAULTS: no_display=true (from %d)", params->no_display);
...
parse_overlay_config(struct overlay_params *params,
                  const char *env, bool use_existing_preset)
{
   mangohud_dbg("PARSE_START: use_existing_preset=%d", use_existing_preset);
   void* bt[24];
   int n = backtrace(bt, 24);
   char** syms = backtrace_symbols(bt, n);
   for (int i = 0; i < n && syms; i++)
      mangohud_dbg("  BT[%d] %s", i, syms[i]);
   if (syms) free(syms);
   ...
   *params = {};
   ...
   params->reset_fps_metrics = { XKB_KEY_Shift_R, XKB_KEY_F8};
   params->open_menu = { XKB_KEY_Shift_R, XKB_KEY_F9 };
   ...
   params->reset_fps_metrics = { VK_F8};
   params->open_menu = { VK_F9 };
   ...
   #define parse_open_menu(x)          params->open_menu
#endif
...
std::shared_ptr<overlay_params> try_get_params() {
...
void presets(int preset, struct overlay_params *params, bool inherit) {
   ...
   switch(preset) {
      case 0:
         params->no_display = 1;
         mangohud_dbg("PRESET case0: no_display=1");
         break;
