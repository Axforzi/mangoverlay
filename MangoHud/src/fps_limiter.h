...
class fpsLimiter {
    ...
    public:
        ...
        /* Replace the configured fps limit with a single value. Used by the
         * menu to cycle the common presets; the params vector is rewritten so
         * current_limit() reports the new limit on the next read. */
        void set_limit(float limit) {
            auto& v = get_params()->fps_limit;
            v.assign(1, limit);
            fps_limits_idx = 0;
            target = limit <= 0.0f ? 0 : int64_t(1'000'000'000.0f / limit);
            active = true;
            SPDLOG_DEBUG("Changed fps limit to {}", limit);
        }
