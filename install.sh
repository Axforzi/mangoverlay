...
#     curl -fsSL https://raw.githubusercontent.com/Axforzi/mangoverlay/v0.1.8/install.sh | bash
...
usage() {
    if [ -r "$0" ]; then sed -n '2,30p' "$0" | sed 's/^# \{0,1\}//'
        ...
        info "Usage:"
        printf '   curl -fsSL https://raw.githubusercontent.com/Axforzi/mangoverlay/v0.1.8/install.sh | bash\n'
...
if [ "$SKIP_OVERLAY" -eq 0 ] && [ -z "$PREBUILT_DIR" ]; then
    [ -d "$MANGO_SRC" ] || die "Overlay source not found: $MANGO_SRC. If you run this script in a directory without the project sources, rerun via 'curl -fsSL https://raw.githubusercontent.com/Axforzi/mangoverlay/v0.1.8/install.sh | bash'."
fi
